...
def benchmark_instance(
    case: BenchmarkCase,
    base_config: QIAEAConfig,
    runs: int,
    target_gap: float,
    base_seed: int | None,
) -> InstanceStatistics:
    """Run repeated QIAEA solves for one instance and aggregate quality metrics"""
    ...
    for run_idx in range(runs):
        ...
        result = solve_tsp_qiaea(instance, config=replace(base_config, seed=run_seed))
...
def run_qiaea_mat_benchmark(
    ...
    """Benchmark QIAEA across all configured cases and build overall summary stats"""
    summaries = [
        benchmark_instance(
            case=case,
            base_config=config,
...
def main(argv: list[str] | None = None) -> int:
    """Parse CLI arguments and run the QIAEA benchmark end to end"""
    ...
    parser.add_argument("--n-males", type=int, default=3)
    parser.add_argument("--elitism", type=int, default=2)
    parser.add_argument("--substring-fraction", type=float, default=0.20)
    parser.add_argument("--substring-fraction-end", type=float, default=None)
    parser.add_argument("--bits-per-city", type=int, default=16)
    parser.add_argument("--random-tie-break", action="store_true", dest="random_tie_break")
    parser.add_argument("--no-random-tie-break", action="store_false", dest="random_tie_break")
    parser.add_argument("--seed", type=int, default=1234)
    parser.set_defaults(random_tie_break=True)
    ...
    config = QIAEAConfig(
        ...
        substring_fraction=args.substring_fraction,
        substring_fraction_end=args.substring_fraction_end,
        bits_per_city=args.bits_per_city,
        random_tie_break=args.random_tie_break,
        seed=None,
    )

    summaries, overall = run_qiaea_mat_benchmark(
        cases=cases,
        config=config,
        runs=args.runs,
